        ...
        else // not a request to start resizing
        {
                 ...
                 !SendEvent( wxEVT_GRID_LABEL_LEFT_CLICK, row, -1, event ) )
            {
                if ( m_selection )
                {
                    ...
                    if ( selectNewRow )
                    {
                        // Move the cursor to the left most cell of the current
                        // row visible area.
                        int col;
                        ...
                        else
                        {
                            ...
                            CalcGridWindowUnscrolledPosition(0, 0,
                                                             &x, NULL,
                                                             m_gridWin);
                            // TODO: the returned column is not fully visible,
        ...
        switch ( event.GetKeyCode() )
        {
            ...
            case WXK_END:
                if ( m_currentCellCoords != wxGridNoCellCoords )
                {
                    ...
                    {
                        if ( m_selection )
                            m_selection->EditCurrentBlock(m_currentCellCoords,
                                                          wxGridCellCoords(row, col),
                                                          event);
                     ...
                     const wxGridDirectionOperations& diroper)
{
    ...
    if ( expandSelection )
    {
        ...
        else if ( !diroper.IsValid(coords) )
        {
            ...
            // is not valid. It mean we can't change the selection block
            // in this direction.
            return false;
        }
        ...
        // TODO: Provide the keyboard state.
        m_selection->EditCurrentBlock(m_currentCellCoords, coords, wxKeyboardState());
    }
